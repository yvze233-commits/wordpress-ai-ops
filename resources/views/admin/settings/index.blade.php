...
@php
    $roleLabels = ['writing' => ['title' => '生成 AI', 'eyebrow' => 'Article generation', 'hint' => '负责根据选题、知识库和图片备注生成文章。'], 'review' => ['title' => '审核 AI', 'eyebrow' => 'Article review', 'hint' => '负责检查事实、结构、证据和发布风险。']];
@endphp

@section('content')
    <div class="page-heading">
        <div><div class="eyebrow">AI setup</div><h1>系统配置</h1><p class="subtitle">分别配置写作和审核使用的 AI。每张卡保存一次，模型只能从列表中选择。</p></div>
    ...
    <form method="post" action="{{ route('admin.settings.update') }}">
        ...
        <div class="settings-grid settings-grid-compact">
            <section class="panel settings-section"><div class="panel-header"><div><h2>内容策略</h2><p>控制每日生产量、写作 Skill 和二段审核门槛。</p></div></div><div class="panel-body form-grid"><label><span>每日文章目标</span><input type="number" min="1" max="20" name="topic_daily_target" value="{{ old('topic_daily_target', $settings['topic_daily_target']) }}"><small>建议保持在 3-5 篇。</small></label><label><span>审核通过分数</span><input type="number" min="1" max="100" name="review_pass_threshold" value="{{ old('review_pass_threshold', $settings['review_pass_threshold']) }}"><small>低于此分数会进入人工复核。</small></label><label><span>写作 Skill</span><select name="writing_skill_slug">@foreach ($writingSkills as $skill)<option value="{{ $skill->slug }}" @selected(old('writing_skill_slug', $settings['writing_skill_slug']) === $skill->slug)>{{ $skill->name }}</option>@endforeach</select><small>新批次会保存所选 Skill 版本。</small></label><label><span>审核 Skill</span><input value="GEOFlow 二段审核：事实与证据 → 结构与发布风险" readonly><small>每篇文章连续调用审核 AI 两次。</small></label></div></section>
    ...
    <script>
        const fillModels = async (form, announce = false) => {
            ...
            const response = await fetch(`{{ route('admin.settings.models') }}?provider=${encodeURIComponent(provider)}`, { headers: { Accept: 'application/json' } });
            ...
            status.textContent = announce ? '模型已更新，可从列表选择' : `${Object.keys(payload.models || {}).length} 个可用模型`;
        };
